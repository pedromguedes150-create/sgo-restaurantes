...
| 9 — Pessoas (API RH) | 🟡 Integração RH ATIVA e testada: sync de colaboradores por unidade (Unit.rhUnitName ↔ razão social; /api/rh/sync). Campos RH: matricula/nome/cpf/status/unidade/cargo/admissao (envelope {data:[]}). **Mapa de Funções 9.3** ✅ (alocado no SGO, alimentado pelo RH — a escala do RH NÃO traz turno/setor). Pendente: rota de Férias (confirmar), comissões/mobilidade (financeiro/folha é agregado), colaborador/:id (id ≠ matrícula). POPs com vídeo YouTube ✅ |
