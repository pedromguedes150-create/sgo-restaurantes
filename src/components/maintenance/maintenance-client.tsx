...
export function MaintenanceClient({ view, isAdmin, units, equipment, suppliers, summary, tickets, plans }: {
  ...
  const tabClass = (t: string) => t === tab
    ? 'inline-flex items-center gap-1 rounded-full bg-primary px-3.5 py-1.5 text-sm font-semibold text-primary-foreground'
    : 'inline-flex items-center gap-1 rounded-full border px-3.5 py-1.5 text-sm font-medium';

  return (
    <div className="space-y-4">
      <h1 className="flex items-center gap-2 text-xl font-bold text-brand"><Wrench className="h-5 w-5 text-accent" /> Manutenção</h1>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <Card><CardContent className="py-3 text-center"><p className="text-2xl font-black text-brand">{summary.open}</p><p className="text-xs text-muted-foreground">abertos</p></CardContent></Card>
        <Card><CardContent className="py-3 text-center"><p className="text-2xl font-black text-brand">{summary.inProgress}</p><p className="text-xs text-muted-foreground">em andamento</p></CardContent></Card>
        <Card><CardContent className="py-3 text-center"><p className="text-2xl font-black text-critical">{summary.overdue}</p><p className="text-xs text-muted-foreground">atrasados</p></CardContent></Card>
        <Card><CardContent className="py-3 text-center"><p className="text-2xl font-black text-success">{summary.doneMonth}</p><p className="text-xs text-muted-foreground">feitos no mês</p></CardContent></Card>
      </div>
      {summary.costMonth > 0 && <p className="text-sm text-muted-foreground">Custo de manutenção no mês: <b className="text-brand">{formatBRL(summary.costMonth)}</b></p>}

      <NewTicket units={units} equipment={equipment} suppliers={suppliers} onDone={onDone} />

      <div className="space-y-2">
        {tickets.length === 0 && <p className="text-sm text-muted-foreground">Nenhum chamado registrado.</p>}
    ...
    <div className="rounded-lg border bg-card p-3">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{t.unit}{t.equipmentName ? ` · ${t.equipmentName}` : ''}{t.supplierName ? ` · ${t.supplierName}` : ''}</p>
          {t.description && <p className="mt-1 text-sm">{t.description}</p>}
          {t.deadline && <p className={`mt-1 text-xs ${isOverdue(t.deadline) && t.status !== 'DONE' && t.status !== 'CANCELED' ? 'font-semibold text-critical' : 'text-muted-foreground'}`}>Prazo: {fmtDate(t.deadline)}{isOverdue(t.deadline) && t.status !== 'DONE' && t.status !== 'CANCELED' ? ' (atrasado)' : ''}</p>}
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        ...
        {(t.status === 'OPEN' || t.status === 'IN_PROGRESS') && <Button size="sm" variant="ghost" className="text-critical" disabled={busy} onClick={() => { if (confirm('Cancelar este chamado?')) act({ action: 'cancel' }); }}><X className="h-4 w-4" /> Cancelar</Button>}
...
function PlansTab({ isAdmin, units, equipment, plans, onDone }: { isAdmin: boolean; units: UnitDTO[]; equipment: EquipDTO[]; plans: PlanDTO[]; onDone: () => void }) {
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">Planos recorrentes por equipamento. Quando vencem, gerente e supervisão são avisados na Central de Notificações.</p>
      <NewPlan units={units} equipment={equipment} onDone={onDone} />
      <div className="space-y-2">
        {plans.length === 0 && <p className="text-sm text-muted-foreground">Nenhum plano preventivo.</p>}
...
function PlanCard({ p, isAdmin, onDone }: { p: PlanDTO; isAdmin: boolean; onDone: () => void }) {
  ...
  return (
    <div className={`rounded-lg border bg-card p-3 ${!p.active ? 'opacity-60' : ''}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="font-semibold text-brand">{p.title}</p>
          <p className="text-xs text-muted-foreground">{p.unit}{p.equipmentName ? ` · ${p.equipmentName}` : ''} · a cada {p.frequencyDays} dias</p>
          <p className={`mt-1 text-xs ${due && p.active ? 'font-semibold text-critical' : 'text-muted-foreground'}`}>Próxima: {fmtDate(p.nextDueAt)}{due && p.active ? ' (vencida)' : ''}{p.lastDoneAt ? ` · última: ${fmtDate(p.lastDoneAt)}` : ''}</p>
          {p.logs.length > 0 && <p className="mt-1 text-xs text-muted-foreground">Histórico: {p.logs.map((l) => fmtDate(l.doneAt)).join(', ')}</p>}
        </div>
