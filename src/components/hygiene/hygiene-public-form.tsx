...
export function HygienePublicForm({ unitId, locations, preselect }: { unitId: string; locations: { id: string; name: string }[]; preselect: string | null }) {
  ...
  if (done) {
    return (
      <div className="rounded-2xl bg-surface p-6 text-center">
        ...
        <p className="text-lg font-bold text-ink-900">Obrigado! 🙏</p>
  ...
  return (
    <div className="space-y-4 rounded-2xl bg-surface p-4">
      {locations.length > 0 && (
        <div>
          <p className="mb-1.5 text-sm font-semibold text-ink-900">Qual banheiro?</p>
      ...
      <div>
        <p className="mb-1.5 text-sm font-semibold text-ink-900">O que está faltando? <span className="font-normal text-ink-500">(opcional)</span></p>
        <div className="flex flex-wrap gap-2">
          {ISSUES.map((it) => (
      ...
      <div>
        <p className="mb-1.5 text-sm font-semibold text-ink-900">Como você avalia este banheiro? <span className="font-normal text-ink-500">(opcional)</span></p>
