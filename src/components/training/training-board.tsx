    ...
    <div className="space-y-4">
      {isAdmin && (
        <div className="flex items-end gap-2 rounded-lg border border-dashed p-2">
          <div><label className="text-xs text-ink-500">Peso de Treinamentos na meta</label><Input inputMode="numeric" value={w} onChange={(e) => setW(e.target.value)} className="h-9 w-24 text-sm" /></div>
      ...
      {board.map((g) => (
        <div key={g.sector} className="space-y-2">
          ...
          {g.collaborators.length === 0 && <p className="text-xs text-ink-500">Nenhum colaborador.</p>}
        </div>
      ))}
    </div>
  );
