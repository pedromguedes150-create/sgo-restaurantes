  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-2">
        ...
        {delegations.map((d) => (
          <div key={d.id} className="flex items-center justify-between rounded-lg border bg-sgo-surface p-3">
            <div><p className="font-semibold text-sgo-brand">{d.from} → {d.to}</p><p className="text-xs text-ink-500">{d.period}</p></div>
  ...
  return (
    <div className="rounded-lg border bg-sgo-surface p-3">
      <div className="flex items-center justify-between gap-2">
        <div><p className="font-semibold text-sgo-brand">{f.name}</p><p className="text-xs text-ink-500">{formatBRL(f.defaultValue)} · PIX: {f.pixKey || <span className="text-danger">não cadastrada</span>} · {f.units.join(', ')}{f.sectorRates.length > 0 ? ` · ${f.sectorRates.length} setor(es) c/ valor-dia` : ''}</p></div>
...
function MiscTypeItem({ m, onChange }: { m: MiscTypeRow; onChange: () => void }) {
  ...
  return (
    <div className="rounded-lg border bg-sgo-surface p-3">
      <div className="flex items-center justify-between gap-2">
        <div><p className="font-semibold text-sgo-brand">{m.name}</p><p className="text-xs text-ink-500">aprova: {ROLE_OPTIONS.find((r) => r.value === m.approverRole)?.label}</p></div>
        <div className="flex items-center gap-1">
