  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {checklists.length === 0 && <p className="text-sm text-muted-foreground">Nenhum checklist criado. Eles são usados pelo supervisor na visita às unidades.</p>}
        {checklists.map((c) => (
          <div key={c.id} className="rounded-lg border bg-card p-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="flex items-center gap-1.5">
                ...
                <Button size="sm" variant="ghost" className="text-critical" disabled={busy} onClick={() => { if (confirm(`Excluir "${c.name}"? (com histórico de visitas, só inativa)`)) void run({ entity: 'supervisorChecklist', action: 'delete', id: c.id }); }} aria-label="Excluir"><Trash2 className="h-4 w-4" /></Button>
              </span>
            </div>
...
function ChecklistForm({ init, busy, onSave, onCancel }: {
  ...
  return (
    <div className="rounded-lg border border-dashed p-3">
      <p className="mb-2 text-xs font-bold uppercase tracking-wide text-muted-foreground">{init ? 'Editar checklist' : 'Novo checklist'}</p>
      <div className="space-y-2">
        ...
        {items.map((it, i) => (
          <div key={i} className="flex gap-1.5">
            ...
            <Button size="sm" variant="ghost" className="text-critical" onClick={() => setItems((s) => s.filter((_, j) => j !== i))} disabled={items.length === 1} aria-label="Remover item"><X className="h-4 w-4" /></Button>
          </div>
        ))}
