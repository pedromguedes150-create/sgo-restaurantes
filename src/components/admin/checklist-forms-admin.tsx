  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {forms.length === 0 && <p className="text-sm text-muted-foreground">Nenhuma ficha cadastrada ainda.</p>}
        {forms.map((f) => {
            ...
            <div key={f.id} className="rounded-lg border bg-card">
              <button onClick={() => setOpenId((id) => (id === f.id ? null : f.id))} className="flex w-full items-center justify-between gap-2 p-3 text-left">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{unitName} · {f.fields} campo(s) · {f.submissions} envio(s)</p>
                </div>
...
function FormEditor({ id, onChanged }: { id: string; onChanged: () => void }) {
  ...
  if (!d) return <div className="border-t p-3 text-sm text-muted-foreground"><Loader2 className="mr-1 inline h-4 w-4 animate-spin" /> Carregando…</div>;
  ...
  return (
    <div className="space-y-4 border-t p-3">
      ...
      <div className="rounded-lg border bg-surface p-2.5">
        <p className="mb-1 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-muted-foreground"><Link2 className="h-3.5 w-3.5" /> Link público</p>
        {d.linkEnabled ? (
        ...
        ) : <p className="text-xs text-muted-foreground">Link desligado — ninguém consegue abrir. Ligue em &quot;Link habilitado&quot;.</p>}
      </div>

      {/* Campos */}
      <div>
        <p className="mb-1 text-xs font-bold uppercase tracking-wide text-muted-foreground">Perguntas / campos</p>
        <div className="space-y-1.5">
          {d.fields.length === 0 && <p className="text-sm text-muted-foreground">Nenhum campo ainda — adicione abaixo.</p>}
          {d.fields.map((f, idx) => (
      ...
      {editing ? (
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            <p className="truncate text-sm font-medium text-brand">{field.kind === 'SECTION' ? `— ${field.label} —` : field.label} {field.required && <span className="text-critical">*</span>}</p>
            <p className="text-xs text-muted-foreground">{kindLabel(field.kind)}{field.options.length ? ` · ${field.options.join(', ')}` : ''}{field.section ? ` · seção: ${field.section}` : ''}</p>
          </div>
          <div className="flex shrink-0 items-center gap-1">
            ...
            <Button size="sm" variant="ghost" className="text-critical" aria-label="Excluir" disabled={busy} onClick={onDelete}><Trash2 className="h-4 w-4" /></Button>
          </div>
        </div>
      )}
    </div>
...
function FieldForm({ initial, onCancel, onSubmit }: { initial?: Field; onCancel: () => void; onSubmit: (patch: Record<string, unknown>) => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {isSelect && <div><Label className="text-xs">Opções (uma por linha)</Label><textarea value={optionsText} onChange={(e) => setOptionsText(e.target.value)} rows={3} className="w-full rounded-lg border-2 border-input bg-background px-3 py-2 text-sm" placeholder={'380g\n300g'} /></div>}
      {!isSection && (
