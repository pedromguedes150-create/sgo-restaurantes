  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="buscar checklist…" className="h-9 w-48 rounded-lg border-2 border-line-strong bg-surface px-3 text-sm" />
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 bg-surface p-2 text-left font-bold text-ink-500">Checklist</th>
              {units.map((u) => (
            ...
            {filtered.map((r) => (
              <tr key={r.name} className="border-t">
                ...
                {units.map((u) => {
                  ...
                  return (
                    <td key={u.id} className={`p-1 text-center ${has ? 'bg-success/70 font-bold text-on-brand' : gap ? 'bg-warning/40 text-warning' : 'text-ink-500'}`} title={`${r.name} · ${u.name}`}>
                      {has ? '✓' : gap ? '!' : '·'}
                    </td>
                  );
                })}
