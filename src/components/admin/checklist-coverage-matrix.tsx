...
export function ChecklistCoverageMatrix({ units, rows }: { units: UnitCol[]; rows: Row[] }) {
  ...
  if (rows.length === 0) return <p className="text-sm text-muted-foreground">Nenhum checklist ativo cadastrado.</p>;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="ml-auto text-xs text-muted-foreground">{filtered.length} de {rows.length} checklist(s)</span>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 bg-card p-2 text-left font-bold text-muted-foreground">Checklist</th>
              {units.map((u) => (
                <th key={u.id} className="p-1 text-center font-semibold text-muted-foreground" title={u.name}>
          ...
          <tfoot>
            <tr className="border-t-2">
              <td className="sticky left-0 z-10 bg-card p-2 text-right font-bold text-muted-foreground">Total por unidade</td>
              {units.map((u) => {
