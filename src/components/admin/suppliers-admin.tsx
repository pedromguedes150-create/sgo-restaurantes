...
export function SuppliersAdmin({ suppliers }: { suppliers: SupplierRow[] }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">Lista compartilhada de fornecedores — usada em Gás, Notas Recebidas e Pagamentos. Alimentada por Admin, CEO e Supervisão.</p>
      ...
      <div className="space-y-2">
        ...
        {suppliers.length === 0 && <p className="text-sm text-muted-foreground">Nenhum fornecedor cadastrado.</p>}
      </div>
    </div>
  );
}
    ...
    <div className="rounded-lg border bg-card p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button size="sm" variant="ghost" className="text-critical" disabled={busy} onClick={() => { if (confirm(`Excluir o fornecedor "${s.name}"? Só é possível se não houver gás/notas/pagamentos vinculados.`)) call({ action: 'delete', id: s.id }); }} aria-label="Excluir"><Trash2 className="h-4 w-4" /></Button>
        </div>
      </div>
      {editing && (
