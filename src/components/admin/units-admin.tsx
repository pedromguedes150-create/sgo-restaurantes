    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-dashed p-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div><Label>CNPJ <span className="font-normal text-muted-foreground">(casa notas de gás)</span></Label><Input inputMode="numeric" value={cnpj} onChange={(e) => setCnpj(e.target.value)} placeholder="00.000.000/0000-00" /></div>
        </div>
...
function UnitItem({ unit, onChange }: { unit: UnitRow; onChange: () => void }) {
  ...
  return (
    <div className="rounded-lg border bg-card p-3">
      ...
      <p className="text-xs">{unit.cnpj ? <span className="text-muted-foreground">CNPJ {formatCnpj(unit.cnpj)}</span> : <span className="font-medium text-critical">Sem CNPJ — necessário para importar notas de gás</span>}</p>
      ...
      {editing && (
        <div className="mt-2 grid grid-cols-2 gap-2 rounded-lg bg-muted/40 p-2">
          ...
          <div className="col-span-2"><Label className="text-xs">CNPJ <span className="font-normal text-muted-foreground">(casa notas de gás por CNPJ)</span></Label><Input inputMode="numeric" value={cnpj} onChange={(e) => setCnpj(e.target.value)} placeholder="00.000.000/0000-00" className="h-10 text-sm" /></div>
