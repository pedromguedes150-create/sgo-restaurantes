...
export function CommunicationConfirm({ id, requiresResponse }: { id: string; requiresResponse: boolean }) {
  ...
  return (
    <div className="space-y-2 rounded-lg border-2 border-accent/40 bg-accent/5 p-3">
      <p className="text-sm font-semibold text-brand">Confirme que leu este comunicado</p>
      <div>
        <Label className="text-xs">Resposta {requiresResponse ? '(obrigatória)' : '(opcional)'}</Label>
        <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2} className="w-full rounded-lg border-2 border-input bg-background px-3 py-2 text-sm" placeholder="Comentário (ex.: feito, segue foto)" />
      </div>
      <div>
