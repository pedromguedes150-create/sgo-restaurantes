...
const ROOM: Record<Coverage, { floor: string; wall: string; dot: string; label: string }> = {
  ...
  partial: { floor: 'bg-medium/20',  wall: 'shadow-[6px_6px_0_0_rgba(202,138,4,0.45)]', dot: 'bg-medium',  label: 'Parcial' },
  none:    { floor: 'bg-critical/10', wall: 'shadow-[6px_6px_0_0_rgba(185,28,28,0.4)]', dot: 'bg-critical', label: 'Sem cobertura' },
};
...
export function UnitFloorplan({ grid }: { grid: Grid }) {
  ...
  if (rooms.length === 0) {
    return <p className="text-sm text-muted-foreground">Cadastre setores para ver a planta da unidade.</p>;
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setTilt((v) => !v)} className="rounded-full border px-3 py-1 text-xs font-medium hover:border-accent">
            ...
            {rooms.map((r) => (
              ...
              >
                ...
                <div className="mt-1.5 flex flex-wrap gap-1">
                  ...
                  {r.names.length > 6 && <span className="text-[10px] text-muted-foreground">+{r.names.length - 6}</span>}
                  {r.count === 0 && <span className="text-[10px] font-semibold text-critical">vazio</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      <p className="text-center text-[11px] text-muted-foreground">Planta automática (teste). Envie a planta da unidade para posicionarmos cada área no lugar real.</p>
    </div>
  );
}
