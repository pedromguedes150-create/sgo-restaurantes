 ...
 * colaborador × POP × ciclo) e saem os números da rede, por unidade, por
 * treinamento, por função e por colaborador. Nenhum número aqui olha para
 * "todos os POPs do sistema": a base é sempre o que se aplica a cada pessoa.
 *
 ...
 * mensal = o mês atual; único = a versão atual do POP. Agosto concluído não
...
export interface LinhaTreino extends Resumo { popId: string; popTitle: string }
...
export interface Painel {
  ...
  /** Todas as linhas filtradas, uma por atribuição — a tabela detalhada. */
...
export function cicloVigente(l: LinhaTreinamento, mesAtual: string): boolean {
  return l.recurrence === 'MONTHLY' ? l.periodKey === mesAtual : l.popVersion === l.popCurrentVersion;
}
...
export function porTreinamentoDe(linhas: LinhaTreinamento[], hoje: string): LinhaTreino[] {
  return [...agrupar(linhas, (l) => l.popId).values()]
    .map((ls) => ({ popId: ls[0].popId, popTitle: ls[0].popTitle, nome: ls[0].popTitle, ...resumoDe(ls, hoje) }))
    .sort(porAtencao)
    .map(({ nome: _n, ...r }) => r);
}
...
export function porColaboradorDe(linhas: LinhaTreinamento[], hoje: string): LinhaColaborador[] {
  return [...agrupar(linhas, (l) => `${l.collaboratorId}|${l.unitId}`).values()]
    .map((ls) => ({
      ...
      itens: [...ls].sort((a, b) => a.popTitle.localeCompare(b.popTitle, 'pt-BR')),
      ...resumoDe(ls, hoje),
    }))
    .sort(porAtencao)
    .map(({ nome: _n, ...r }) => r);
