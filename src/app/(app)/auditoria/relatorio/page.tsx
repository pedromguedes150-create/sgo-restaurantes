  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2 print:hidden">
        <Link href="/auditoria" className="inline-flex items-center gap-1 text-sm font-medium text-ink-500 hover:text-sgo-brand"><ArrowLeft className="h-4 w-4" /> Auditoria</Link>
      ...
      <div className="flex flex-wrap gap-2 print:hidden">
        {PERIODS.map((d) => (
          <Link key={d} href={q(d, mod)} className={days === d ? 'rounded-full bg-sgo-brand px-3 py-1.5 text-sm font-semibold text-on-brand' : 'rounded-full border px-3 py-1.5 text-sm font-medium'}>{d} dias</Link>
        ))}
        <span className="mx-1 self-center text-ink-500">·</span>
        <Link href={q(days)} className={!mod ? 'rounded-full bg-sgo-brand px-3 py-1.5 text-sm font-semibold text-on-brand' : 'rounded-full border px-3 py-1.5 text-sm font-medium'}>Todos</Link>
        {modules.map((m) => (
          <Link key={m} href={q(days, m)} className={mod === m ? 'rounded-full bg-sgo-brand px-3 py-1.5 text-sm font-semibold text-on-brand' : 'rounded-full border px-3 py-1.5 text-sm font-medium'}>{m}</Link>
        ))}
      </div>

      <div>
        <h1 className="text-xl font-bold text-sgo-brand">Relatório de Auditoria</h1>
