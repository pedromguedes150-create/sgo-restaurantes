  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between print:hidden">
        <Link href="/configuracoes/modelos" className="inline-flex items-center gap-1 text-sm font-semibold text-accent"><ArrowLeft className="h-4 w-4" /> Modelos</Link>
      ...
      {[...groups.entries()].map(([cat, list]) => (
        <section key={cat} className="break-inside-avoid">
          <h2 className="mt-3 border-b pb-1 text-base font-bold text-brand">{cat}</h2>
          {list.map((m) => (
            <div key={m.id} className="mt-2 break-inside-avoid">
              <p className="text-sm font-semibold">{m.moment ?? m.name} <span className="text-xs font-normal text-muted-foreground">({m.scope === 'MANAGER' ? 'individual' : 'da unidade'}{m.limitTime ? ` · limite ${m.limitTime}` : ''} · peso {m.weight})</span></p>
              <ul className="ml-1 mt-1 space-y-0.5">
                {m.items.map((it) => (
                  <li key={it.id} className="flex items-start gap-2 text-sm">
                    <span className="mt-0.5 inline-block h-3.5 w-3.5 shrink-0 rounded border" />
                    <span>{it.text}{it.requiresPhoto && <Camera className="ml-1 inline h-3 w-3 text-muted-foreground" />}</span>
                  </li>
                ))}
