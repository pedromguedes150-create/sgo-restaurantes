  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between print:hidden">
        <Link href="/configuracoes/modelos" className="inline-flex items-center gap-1 text-sm font-semibold text-brand"><ArrowLeft className="h-4 w-4" /> Modelos</Link>
        <PrintButton label="Imprimir / Salvar PDF" />
      </div>
      <h1 className="text-xl font-bold text-ink-900">Biblioteca de modelos de checklist</h1>

      {[...groups.entries()].map(([cat, list]) => (
        <section key={cat} className="break-inside-avoid">
          <h2 className="mt-3 border-b pb-1 text-base font-bold text-ink-900">{cat}</h2>
          {list.map((m) => (
