    ...
    <div className="space-y-4">
      ...
      <Card><CardContent className="pt-4">
        <PaymentsAdmin
          units={units}
          users={users}
          freelancers={freelancers.map((f) => ({ id: f.id, name: f.name, defaultValue: Number(f.defaultValue), active: f.active, units: f.units.map((u) => u.unit.name), unitIds: f.units.map((u) => u.unit.id) }))}
