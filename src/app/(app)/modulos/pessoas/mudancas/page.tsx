...
export default async function MudancasPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h1 className="flex items-center gap-2 text-xl font-bold text-brand"><ArrowRightLeft className="h-5 w-5 text-accent" /> Mudanças de função/setor</h1>
      ...
      <Card>
        <CardContent className="pt-4">
          {rows.length === 0 && <p className="text-sm text-muted-foreground">Nenhuma mudança registrada ainda. Edite função/setor no Mapa de Funções.</p>}
              ...
              <div key={r.id} className="rounded-lg border bg-card p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm">
                      <span className="text-muted-foreground">{r.fromValue || 'sem registro'}</span>
                      {' → '}
