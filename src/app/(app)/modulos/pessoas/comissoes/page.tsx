...
export default async function ComissoesPage({ searchParams }: { searchParams: { mes?: string } }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h1 className="flex items-center gap-2 text-xl font-bold text-brand"><HandCoins className="h-5 w-5 text-accent" /> Comissões &amp; Mobilidade</h1>
        <p className="text-sm text-muted-foreground">Supervisão/Admin lançam os valores (comissão do Teknisa / mobilidade) por colaborador. Dashboard e histórico mensal.</p>
      </div>
      <Card>
