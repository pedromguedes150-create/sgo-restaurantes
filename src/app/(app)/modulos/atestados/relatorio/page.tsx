  ...
  return (
    <div className="sgo-print mx-auto max-w-3xl space-y-4 bg-surface p-2 text-ink-900 print:p-0">
      <div className="flex items-center justify-between gap-2 print:hidden">
        <Link href={`/modulos/atestados?mes=${ym}`} className="inline-flex items-center gap-1 text-sm font-semibold text-brand"><ArrowLeft className="h-4 w-4" /> Voltar</Link>
        <div className="flex gap-2">
          <a href={`/api/certificates/export?mes=${ym}`} className="inline-flex items-center gap-1 rounded-full border px-3 py-1.5 text-sm font-semibold text-brand"><Download className="h-4 w-4" /> Excel/CSV</a>
          <PrintButton />
        </div>
      </div>
      ...
      <div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {report.byUnit.length === 0 && <tr><td colSpan={4} className="py-2 text-ink-500">Sem atestados no mês.</td></tr>}
            {report.byUnit.map((u) => (
              <tr key={u.unitId} className="border-b border-line"><td className="py-1">{u.unitName}</td><td className="py-1 text-right">{u.count}</td><td className="py-1 text-right">{u.days}</td><td className="py-1 text-right">{u.absenteeismPct}%</td></tr>
            ))}
          </tbody>
        </table>
      </div>
      ...
      <div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {rows.length === 0 && <tr><td colSpan={showCid ? 6 : 5} className="py-2 text-ink-500">—</td></tr>}
            {rows.map((r) => (
      ...
      <p className="pt-4 text-center text-[10px] text-ink-500">Gerado pelo SGO Beija Flor{showCid ? '' : ' · CID omitido (dado sensível LGPD)'}</p>
    </div>
  );
}
