...
import { preenchimentoDoMes, resumoDoMes } from '@/lib/schedule/materializar';
...
export default async function EscalaPage({ searchParams }: { searchParams: { unit?: string; year?: string; month?: string } }) {
  ...
  const [grid, collaborators, turnos, tipos, legadas, fill, resumo] = await Promise.all([
    ...
    resumoDoMes(selected.id, year, month),
  ]);
    ...
    <div className="space-y-4">
      ...
      <Card><CardContent className="pt-4">
        <ScheduleClient
          ...
          podeLimpar={user.role === 'ADMIN'}
          nomeDaUnidade={selected.name}
          resumoDoMes={resumo}
          units={units}
