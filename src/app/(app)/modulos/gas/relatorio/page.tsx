  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between print:hidden">
        <Link href="/modulos/gas" className="inline-flex items-center gap-1 text-sm font-semibold text-sgo-brand"><ArrowLeft className="h-4 w-4" /> Gás</Link>
        <div className="flex gap-2">
          <a href="/api/gas/export" className="inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-sm font-semibold hover:border-sgo-brand"><Download className="h-4 w-4" /> Excel</a>
          <PrintButton label="PDF" />
        </div>
      </div>
      <h1 className="text-xl font-bold text-sgo-brand">Variação do preço do gás (R$/kg)</h1>
        ...
        <Card key={u.unitId} className="break-inside-avoid">
          <CardContent className="space-y-2 pt-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h2 className="font-bold text-sgo-brand">{u.unit}</h2>
              ...
              <tbody>
                {u.rows.map((r, i) => (
                  <tr key={i} className="border-b last:border-0">
                    ...
                    <td className={`py-1 text-right font-medium ${r.variationPct == null ? 'text-ink-500' : r.variationPct > 0 ? (r.alerted ? 'text-danger' : 'text-warning') : 'text-sgo-success'}`}>
