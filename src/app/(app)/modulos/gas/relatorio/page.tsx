  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between print:hidden">
        <Link href="/modulos/gas" className="inline-flex items-center gap-1 text-sm font-semibold text-accent"><ArrowLeft className="h-4 w-4" /> Gás</Link>
        <div className="flex gap-2">
          <a href="/api/gas/export" className="inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-sm font-semibold hover:border-accent"><Download className="h-4 w-4" /> Excel</a>
          <PrintButton label="PDF" />
        </div>
      </div>
      <h1 className="text-xl font-bold text-brand">Variação do preço do gás (R$/kg)</h1>
      <p className="text-sm text-muted-foreground">Histórico dos recebimentos (12 meses), com a variação do preço unitário a cada compra.</p>

      {units.length === 0 && <p className="text-sm text-muted-foreground">Nenhum recebimento no período.</p>}
              ...
              <tbody>
                {u.rows.map((r, i) => (
                  <tr key={i} className="border-b last:border-0">
                    ...
                    <td className="py-1 pr-2 text-muted-foreground">{r.supplier}</td>
                    <td className="py-1 pr-2 text-right font-semibold">{kg(r.price)}</td>
                    <td className={`py-1 text-right font-medium ${r.variationPct == null ? 'text-muted-foreground' : r.variationPct > 0 ? (r.alerted ? 'text-critical' : 'text-medium') : 'text-success'}`}>
