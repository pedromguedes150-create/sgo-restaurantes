...
export default async function ConferenciaPage({ searchParams }: { searchParams: { unit?: string } }) {
  ...
  if (units.length === 0) return <p className="text-sm text-muted-foreground">Nenhuma unidade vinculada ao seu usuário.</p>;
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-bold text-brand">
            <ScanLine className="h-5 w-5 text-accent" /> Conferência por leitor
          </h1>
        ...
        {!isCashier && (
          <Link href="/modulos/comandas" className="inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-sm font-semibold hover:border-accent">
            ← Contagem manual
          </Link>
        )}
      </div>
