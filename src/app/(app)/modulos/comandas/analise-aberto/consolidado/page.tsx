  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 print:max-w-none">
      <div className="flex items-center justify-between gap-2 print:hidden">
        <Link href="/modulos/comandas/analise-aberto" className="inline-flex items-center gap-1 text-sm font-semibold text-accent"><ArrowLeft className="h-4 w-4" /> Voltar</Link>
      ...
      <div className="rounded-lg border p-4 print:border-0 print:p-0">
        <div className="mb-3 border-b pb-2">
          ...
          <p className="text-sm text-muted-foreground">Para o Administrativo · gerado {new Date().toLocaleString('pt-BR')} · baseado na análise mais recente de cada unidade</p>
          <p className="text-sm"><b>{totalCmds}</b> comanda(s) a travar em <b>{rows.length}</b> unidade(s) · valor total <b>{formatBRL(totalValue)}</b></p>
        </div>

        {rows.length === 0 && <p className="text-sm text-muted-foreground">Nenhuma análise com suspeitas. Suba os relatórios em cada unidade primeiro.</p>}

        <div className="space-y-4">
          {rows.map((r) => (
            <div key={r.unitId} className="break-inside-avoid">
              <p className="text-sm font-bold text-brand">{r.unitName} <span className="font-normal text-muted-foreground">· corte {fmt(r.cutDate)} · {r.suspects.length} comanda(s) · {formatBRL(r.suspectValue)}</span></p>
              <table className="mt-1 w-full border-collapse text-xs">
                <thead><tr className="border-b text-left text-muted-foreground"><th className="p-1">Comanda (travar)</th><th className="p-1">Aberta em</th><th className="p-1 text-center">Dias</th><th className="p-1 text-right">Valor</th></tr></thead>
                <tbody>
                  {r.suspects.map((s, i) => (
