...
export async function GET() {
  ...
  const buf = exportProductsBuffer(products.map((p) => ({ name: p.name, origin: p.origin, category: p.category, measure: p.measure, packSize: p.packSize, barcode: p.barcode, active: p.active })));
