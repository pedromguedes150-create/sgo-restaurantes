...
- **Módulo Estoque** (`/modulos/estoque`, `src/lib/stock/*`). Controle de prateleira pelo celular: bipar → informar o que encontrou → seguir. **Não é registro fiscal**, e o desenho inteiro sai daí.
  - **O saldo é DECLARADO, nunca deduzido.** Não há PDV integrado; inventar uma baixa por venda produziria um número exato e falso, pior que um aproximado e assumido. Toda mudança vira `StockMovement` com autor e hora.
  - **A TRATATIVA é o coração** — é ela que substitui o PDV. Quando um lote entra na faixa de alerta, o SGO pergunta e grava a resposta: ✅ Lote finalizado · 📦 Ainda possui estoque (única que pede quantidade e mantém o lote vivo) · 🗑️ Descarte/perda · 🔄 Transferido. Sem isso, ou o gerente viraria digitador de saída diária, ou o alerta repetiria a mesma pergunta para sempre sobre um lote que acabou há um mês. Respondida uma faixa, a pergunta só volta quando o lote **cruza para a faixa seguinte** (`StockLot.lastReviewBand`).
  - **Validade por LOTE, não por produto** (`StockLot`): o mesmo molho chega hoje vencendo em dezembro e na próxima entrega vencendo em fevereiro. Duas entregas do mesmo lote **somam** numa linha só — na prateleira é uma pilha, e duas linhas fariam responder a mesma pergunta duas vezes.
  - **A faixa é CALCULADA na leitura, jamais gravada**: 30 🟡 · 7 🟠 · 2 🔴 · vence hoje 🔴 · vencido ⛔. O dia vira sozinho — um status guardado diria "vence em 7 dias" três semanas depois, e o número sairia plausível (mesma armadilha da variação do gás, v1.103.0). A **antecedência do primeiro aviso é do PRODUTO** (`alertDays`, padrão 30): produto de 15 dias e produto de 6 meses não podem acender juntos. A escalada seguinte é **fixa e igual para todos** de propósito — "crítico" precisa querer dizer a mesma coisa em toda unidade. E janela curta **não cala** as faixas graves: a 2 dias do vencimento é crítico, tenha o produto a antecedência que tiver.
  - **Embalagem**: `PackType` UN/FARDO/DISPLAY + quantas unidades vêm dentro. O saldo mora **sempre em unidades** (guardar as duas grandezas seriam dois números que um dia discordam); a tela mostra as duas leituras — "5 fardos · 60 un" — e a conversão aparece **antes** de salvar. O fator vai **congelado no lote**: corrigir o cadastro não pode reescrever quanto tinha um lote de três meses atrás.
  - **Um cadastro, N códigos de barras.** Nenhuma tabela nova: `Product` já é o catálogo da rede e `ProductBarcode.code` já é `@unique`, o que torna a duplicidade impossível por construção. Código novo → o SGO oferece **vincular** ao produto existente em vez de criar um segundo cadastro (que criaria dois saldos para a mesma prateleira). A ordem da busca é o código exato primeiro e a semelhança de nome depois: o gerente está de pé no estoque, e o índice responde em milissegundos.
  - **O gerente cadastra produto novo**, da prateleira, para a rede inteira.
  - **O SETOR DO CD É SUGERIDO** (`src/lib/stock/setor-sugerido.ts` puro + `src/lib/ai/produto-setor.ts`): uma Coca é Bebidas, um chiclete é Bomboniere, uma coxinha é Salgados. **Duas camadas, e a ordem é o desenho**: a regra determinística resolve o romaneio inteiro na hora, de graça e **sem chave de IA**; a IA entra só no que sobra. Perguntar à IA o que uma tabela responde em microssegundos gastaria segundos com o gerente de pé no estoque, e deixaria o cadastro dependente de rede.
  - **A ORIGEM passou a ser DERIVADA do setor**: com setor confirmado o produto nasce `CD` e já é pedível; sem setor nasce `LOCAL` e fica só no estoque. O que ficou **inalcançável** é nascer `CD` sem setor — o item sumiria da fila de todos os separadores sem erro nenhum (defeito da v1.94.0). Não há como a tela pedir CD e esquecer o setor.
  - As regras foram **calibradas contra o romaneio real do CD**, item por item, e o teste É aquela lista. Ela ensinou três coisas que um catálogo inventado não ensinaria: **o que a coisa é vence do que ela é feita** (coxinha de requeijão é salgado, não laticínio — por comprimento de termo, "requeijão" venceria "coxinha"); **o recipiente vence o conteúdo** (forminha de papel para empada é descartável), mas **"embalagem" não entra como termo**, porque ali é unidade de medida ("12 embalagens de brigadeiro") e roubaria a confeitaria; e **o plural é a regra** ("Bolos", "Broas", "Molhos de Pimenta", "pães de sal"), o que exigiu reduzir plural dos dois lados — inclusive irregulares (pães→pão, descartáveis→descartável, que também conserta o casamento do nome do setor).
  - ⚠️ **Nenhum nome de setor está fixo no código.** Os setores são cadastrados por instalação; hoje o banco tem "Bebidas, Secos, Descartáveis" e o romaneio tem oito. Cada conceito declara os APELIDOS pelos quais o setor costuma se chamar, e o casamento é com o que está cadastrado — renomear "Secos" para "Despensa Cozinha" não quebra nada. **Conceito sem setor cadastrado não sugere**: apontar um setor "mais ou menos parecido" mandaria o produto para a fila de quem não tem o que fazer com ele.
  - A IA recebe a **lista fechada** dos setores existentes e só pode responder com um deles ou "NENHUM"; a resposta é conferida contra a lista antes de virar sugestão. Inerte sem `ANTHROPIC_API_KEY`.
- **Guia da plataforma** atualizado com o módulo (`/ajuda`), incluindo o que ele NÃO é.
### Alterado
- **`ProductOrigin` ganhou `LOCAL`** (compra direta da unidade). Era necessário: o gerente bipando uma Coca-Cola do distribuidor não tem origem nem setor do CD para informar, e produto do CD sem setor **some da fila de todos os separadores sem avisar ninguém** (v1.94.0). ⚠️ **A esteira da Fábrica/CD não mudou**, e o filtro é explícito em **dois** lugares — `listActiveProducts` (a tela de pedido) e `criarPedido` (a gravação). Filtrar só na tela deixaria a porta aberta para um item local virar um `ProductRequest` de origem que nenhuma aba trata. Os três testes disso foram **verificados reprovando** sem o filtro.
- Migração `20260922141552_estoque_operacional`: **aditiva**, sem `DROP` nem `RENAME` (2 tabelas, 3 enums, 3 colunas em `products`, 1 valor de enum).
### Notas
- 156 testes novos (26 puros de validade/embalagem, 85 de classificação de setor — o romaneio real item por item —, 34 de integração e 11 de render).
- **Fora desta etapa, por combinação**: transferência entre unidades, gancho no recebimento da Fábrica/CD, painel consolidado do administrador e IA de cadastro por FOTO do rótulo (a IA de setor entrou nesta). O valor do estoque em R$ ficou de fora porque **não existe custo por produto** em lugar nenhum do SGO — nem no catálogo, nem nas notas (que têm valor total, não por item); somar reais hoje seria inventar o número.

## v1.104.0 — 2026-09-21 (Gás: as colunas voltam a subir — preço ponderado, faixa de plausibilidade e escala dos gráficos)
### Corrigido
- 🔴 **"As colunas não estão subindo" no painel de gás.** O relato era do gráfico, mas a causa estava nos dados e atingia a tela inteira: **uma nota com preço/kg fora de qualquer faixa real**. Os três números do print fechavam a conta — preço médio de R$ 48,9914/kg ao lado de um último preço de R$ 7,0162/kg, e R$ 2.203.879 para 46 toneladas (o real seria ~R$ 300 mil): **uma nota de ~R$ 1,9 milhão com ~300 kg**, a assinatura do TOTAL da nota lançado na coluna do preço unitário. Como **todo gráfico de gás é escalado pela série**, essa nota sozinha jogava as colunas de todos os meses e as barras de todas as unidades no piso de 4px.
  - **A faixa de plausibilidade** (`precoImplausivel`, teto `GAS_MAX_PRICE_KG` padrão **R$ 30,00/kg**, Admin ajusta no próprio painel): recusa o lançamento manual, a **correção** de lançamento e a linha da **importação por planilha**. O teto não julga se a compra foi cara — ele separa preço de **QUILO** de preço de **OUTRA COISA**: R$ 293 é o botijão P45 inteiro, milhares é o total da nota. É ~4,5× o preço da rede, então nada legítimo chega perto. A recusa **diz o que conferir** ("a quantidade está em KG? o valor é o total da nota?"), porque "inválido" faria reimportar igual.
  - **A importação em lote era a porta aberta**: o modelo diz "Preço unitário = preço POR KG", mas nada verificava — e ali o erro é pior, porque o import ainda **MULTIPLICA** o preço pela quantidade. Agora a linha sai como ERRO **na prévia**, antes de gravar.
  - **As notas já gravadas saem NOMEADAS**: aviso vermelho no topo do painel com unidade, data, preço, kg e valor de cada uma, e o caminho do conserto (corrigir kg/valor no Histórico). Antes a tela apenas ficava estranha e não dizia por quê. **Nada é alterado ou excluído no banco** — quem corrige é a pessoa, pela tela que já existia.
- **O preço médio/kg do painel era uma média SIMPLES dos preços.** Mesmo erro do ticket médio consolidado, e igualmente plausível na tela: dava o mesmo peso a uma compra de 30 kg e a uma de 600 kg. Passou a ser **ponderado** (valor ÷ kg, `mediaPonderada`) no cartão do topo, **por unidade**, **por fornecedor** e no **preço do mês** da tendência. Mín/máx continuam sendo os preços das notas — são as pontas, não médias.
- **Os gráficos de preço não mostravam variação, mesmo com dado limpo.** As barras iam de **zero** ao máximo da série; como o gás varia de R$ 6,46 a R$ 7,02, a menor coluna já tinha **92%** da altura da maior — o gráfico que existe para mostrar variação era justamente o que a escondia. A escala passou a ir do menor ao maior da série (com 20% de folga embaixo), o que também tira o poder de uma nota fora de faixa achatar as outras. Série estável cai em meia altura: não há variação a mostrar, e nem "despencou" nem "disparou".
- **O "último preço/kg" dependia da ordem em que o banco devolvia as linhas.** O desempate era `date >=` e a consulta não ordenava por `createdAt` — com **quatro notas no mesmo 23/07** (é o caso do arquivo real) o número podia mudar entre duas leituras da mesma tela. O `ORDER BY` ganhou o desempate e a agregação passou a usar `ordemCronologica`, a mesma da cadeia de variação.
- 24 testes novos (13 puros + 11 de integração), mais 3 na prévia da importação. Verificado que o caso do preço ponderado **reprova** o código anterior. O caso do desempate **não reproduz** a falha (ela é não-determinística por natureza) e está escrito para fixar o contrato — o comentário do teste diz isso.
